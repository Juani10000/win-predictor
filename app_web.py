...
combo = generar_combinada_inteligente(
    partidos_del_dia,
    df_unificado,
    paquete_ia
)

st.success("Combinada Inteligente del Día")

for i,pick in enumerate(combo["picks"],1):

    st.markdown(
        f"""
        **{i}. {pick['descripcion']}**

        Probabilidad: **{pick['prob']:.1f}%**

        Cuota Justa: **{pick['cuota']}**
        """
    )
...
st.metric(
    "Probabilidad de acertar toda la combinada",
    f"{combo['probabilidad']}%"
)
...
def generar_combinada_inteligente(partidos_del_dia, df_unificado, paquete_ia):
    ...
    for partido in partidos_del_dia:
        ...
        xg_local = round(float(row_loc["xG"]) * (1 + FACTOR_LOCALIA), 2)
        xg_vis = round(float(row_vis["xG"]) * (1 - FACTOR_LOCALIA * 0.5), 2)

        stats_loc = consolidar_estadisticas(local, df_unificado, xg_local)
        stats_vis = consolidar_estadisticas(visitante, df_unificado, xg_vis)

        historial = obtener_historial_directo(local, visitante)

        p_loc, p_emp, p_vis, _ = realizar_prediccion(
            local,
            visitante,
            df_unificado,
            stats_loc,
            ...
            xg_local,
            xg_vis,
            historial_h2h=historial,
            paquete_ia=paquete_ia
        )

        over25, under25, btts = calcular_mercados_adicionales(
            xg_local,
            xg_vis
        )
        ...
        opciones = [
            ...
            {
                "descripcion": f"Gana {local}",
                "prob": p_loc,
                "cuota": round(100 / max(1, p_loc),2)
            },

            {
                "descripcion": f"Gana {visitante}",
                "prob": p_vis,
                "cuota": round(100 / max(1,p_vis),2)
            },

            {
                "descripcion":"Más de 2.5 goles",
                "prob":over25,
                "cuota":round(100/max(1,over25),2)
            },

            {
                "descripcion":"Ambos anotan",
                "prob":btts,
                "cuota":round(100/max(1,btts),2)
            }
        ...
        combinadas.append(mejor)

    combinadas.sort(key=lambda x:x["prob"],reverse=True)

    seleccion = combinadas[:3]

    prob_total = 1

    cuota_total = 1

    for pick in seleccion:

        prob_total *= pick["prob"]/100
    ...
    return {
        ...
        "probabilidad":round(prob_total*100,1),
